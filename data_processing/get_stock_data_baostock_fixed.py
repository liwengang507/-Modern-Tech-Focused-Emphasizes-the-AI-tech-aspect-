...
class BaoStockDataCollector:
    ...
    def get_daily_data(self, stock_codes, start_date='2020-01-01', end_date=None, max_stocks=100):
        """
        获取日线数据（限制数量以避免Excel文件过大）
        ...
        print(f"限制获取前 {max_stocks} 只股票以避免文件过大")
        
        all_daily_data = []
        
        # 限制股票数量
        limited_codes = stock_codes[:max_stocks]
        
        for i, code in enumerate(limited_codes):
            try:
                print(f"正在获取股票 {code} 的日线数据 ({i+1}/{len(limited_codes)})")
    ...
    def collect_all_data(self, start_date='2020-01-01', end_date=None):
        ...
        stock_codes = stock_basic['code'].head(500).tolist()  # 限制为500只股票
        ...
        # 获取日线数据（限制数量）
        daily_data = self.get_daily_data(stock_codes, start_date, end_date, max_stocks=100)
...
def main():
    """主函数"""
    print("A股上市公司年报数据获取程序 (基于BaoStock - 修复版)")
    print("=" * 70)
    print("特点：按IPO日期排序，解决Excel文件大小限制问题")
    print("=" * 70)
    ...
    try:
        ...
        if data_dict:
            ...
            filename = f'A股上市公司数据_BaoStock_按IPO排序_{timestamp}.xlsx'
